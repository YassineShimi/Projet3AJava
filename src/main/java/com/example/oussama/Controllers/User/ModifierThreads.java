...
public class ModifierThreads {
    ...
    @FXML
    public void initialize(Threads threads) {
        this.threads=threads;
        TitleF.setText(threads.getTitle());
        authorF.setText(threads.getAuthor());
        topicF.setText(threads.getTopic());
    }
    boolean checkInpute(){
    ...
    void modifier(){
        checkInpute();
        if(checkInpute()){
            this.threads.setTitle(TitleF.getText());
            this.threads.setAuthor(authorF.getText());
            this.threads.setTopic(topicF.getText());
            this.threads.setLikes(0);
            try {
                threadService.modifier(this.threads);
                showSuccessAlert("Threads ajoutée avec succès.");
                ...
                showErrorAlert("Erreur lors de l'ajout de Threads : " + e.getMessage());
